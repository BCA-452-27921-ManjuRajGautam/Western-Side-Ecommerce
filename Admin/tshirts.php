...
// Total tshirt stock
$stock_q = "SELECT SUM(stock) as total FROM products WHERE category='tshirt' AND status='active'";
$stock_r  = mysqli_query($conn, $stock_q);
$total_stock = ($stock_r) ? mysqli_fetch_assoc($stock_r)['total'] : 0;

// Total tshirts sold (confirmed orders)
...
// Total tshirt products count
$count_q = "SELECT COUNT(*) as total FROM products WHERE category='tshirt'";
$count_r  = mysqli_query($conn, $count_q);
$total_products = ($count_r) ? mysqli_fetch_assoc($count_r)['total'] : 0;

// --- ALL TSHIRT PRODUCTS ---
$products_q = "SELECT * FROM products WHERE category='tshirt' ORDER BY id DESC";
...
<head>
    ...
    <title>Western-Side | T-Shirts Admin</title>
    ...
    <style>
        ...
        .stat-stock   .stat-icon { background: linear-gradient(135deg, #ff6b6b, #ff8e8e); color: white; }
        .stat-sold    .stat-icon { background: linear-gradient(135deg, #4ecdc4, #44a08d); color: white; }
        .stat-revenue .stat-icon { background: linear-gradient(135deg, #f093fb, #f5576c); color: white; }
        .stat-total   .stat-icon { background: linear-gradient(135deg, #45b7d1, #96c93d); color: white; }
...
<div class="sidebar" id="sidebar">
    ...
    <nav class="nav flex-column mt-2">
        ...
        <a class="nav-link active" href="tshirts.php">
        ...
        <div class="row align-items-center">
            <div class="col-md-6">
                <h1 style="color: #1e3c72; font-family: 'Delius Swash Caps', cursive;">
                    <i class="fas fa-tshirt"></i> T-Shirts Collection
                </h1>
                <p class="mb-0 text-muted">Manage all men's t-shirts inventory</p>
            </div>
    ...
    <div class="table-responsive">
        <div class="table-header d-flex justify-content-between align-items-center">
            <h4 style="color: #1e3c72; margin: 0;">
                <i class="fa fa-list mr-2"></i> T-Shirts Inventory
            </h4>
            <a href="add_product.php" class="btn btn-admin text-white">
                <i class="fa fa-plus"></i> Add New T-Shirt
            </a>
        </div>
            ...
            <?php else: ?>
                <tr>
                    <td colspan="8" class="text-center text-muted py-5">
                        <i class="fa fa-tshirt fa-3x mb-3 d-block"></i>
                        No T-Shirts found. <a href="add_product.php">Add one now</a>
                    </td>
                </tr>
            <?php endif; ?>
            </tbody>
